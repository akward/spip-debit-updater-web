  ...
  try {
    ...
    for (const job of jobs) {
      try {
          ...
          if (r0302.length) {
            if (job.valueColumn === "jumlah") {
              if (job.name.includes("ATM+Debet")) {
                ...
                if (m.addedDebit)
                  results.push({ job: `${job.name} [LSBU 0302]`, status: "info", reason: `+${m.addedDebit} KARTU_ATM_DEBIT` });
              } else if (job.name.includes("Kartu ATM")) {
                const m = mergeLsbuDebit(r0302, { kartuAtm: map });
                if (m.addedAtm)
                  results.push({ job: `${job.name} [LSBU 0302]`, status: "info", reason: `+${m.addedAtm} KARTU_ATM` });
              }
            }
            for (const rule of DEBIT_TRX_LSBU) {
              if (rule.match.some((m) => job.name === m)) {
                ...
                if (n)
                  results.push({ job: `${job.name} [LSBU 0302]`, status: "info", reason: `+${n} ${rule.jenis.join("|")}` });
              }
            }
            if (job.kind === "matrix-atm") {
              ...
              if (extra.length) {
                ...
                results.push({ job: `${job.name} [LSBU 121]`, status: "info", reason: `+${extra.length} id dari 121` });
              }
            }
          }
        }
        ...
        if (group === "ue") {
          ...
          if (r0302.length) {
            for (const rule of UE_LSBU_MAP) {
              ...
              if (n)
                results.push({ job: `${job.name} [LSBU 0302]`, status: "info", reason: `+${n} ${rule.jenis[0]}` });
              break;
            }
          }
        }

        // KK FORMA0301 — notebook parity
        if (group === "kk") {
          const r = lsbuByKind("forma0301");
          if (r.length) {
            if (job.name === "Jumlah Kartu") {
              const n = mergeLsbuKk(r, map);
              if (n) results.push({ job: `${job.name} [LSBU 0301]`, status: "info", reason: `+${n} JUMLAH_KARTU` });
            } else if (job.name === "Jumlah Account") {
              const n = mergeLsbuKkAccount(r, map);
              if (n) results.push({ job: `${job.name} [LSBU 0301]`, status: "info", reason: `+${n} JUMLAH_ACCOUNT` });
            } else if (job.name === "Nilai Outstanding") {
              const n = mergeLsbuKkOutstanding(r, map, job.divideBy);
              if (n) results.push({ job: `${job.name} [LSBU 0301]`, status: "info", reason: `+${n} SUM outstanding` });
            } else if (job.name === "Nilai NPL") {
              const n = mergeLsbuKkNpl(r, map, job.divideBy);
              if (n) results.push({ job: `${job.name} [LSBU 0301]`, status: "info", reason: `+${n} SUM NPL 90-180 DPD` });
            }
          }
        }
            ...
            for (const rule of ACQUIRER_LSBU_JENIS) {
              if (
                ...
                if (n) {
                  ...
                  matrixRows = [...(matrixRows || []), ...extra];
                  results.push({ job: `${job.name} [LSBU 0304]`, status: "info", reason: `+${n} ${rule.jenis}` });
                }
              }
            }
          }
          ...
          if (r0303.length) {
            for (const rule of ACQUIRER_0303_MAP) {
              if (rule.match.some((m) => job.name === m)) {
                ...
                if (n) results.push({ job: `${job.name} [LSBU 0303]`, status: "info", reason: `+${n} ${rule.transaksi}` });
              }
            }
          }
        }
        ...
        if (group === "fraud_bank") {
          ...
          if (r.length) {
            ...
            if (card) {
              ...
              if (n) results.push({ job: `${job.name} [LSBU 0306]`, status: "info", reason: `+${n} ${card}` });
            }
          }
        }
        ...
        if (group === "fraud_penyebab") {
          ...
          if (r.length) {
            ...
            if (card) {
              ...
              if (n) results.push({ job: `${job.name} [LSBU 0306 map]`, status: "info", reason: `+${n} CP/PL/HD/TD/FA/X` });
            }
          }
        }
        ...
        if (!spreadsheetId) {
          results.push({ job: job.name, status: "error", reason: `Env ${job.spreadsheetEnv} belum di-set`, ids: map.size, source });
          continue;
        }
        ...
        if (job.kind === "matrix-atm") {
          if (!matrixRows?.length) {
            results.push({ job: job.name, status: "skip-matrix-no-data", reason: "Butuh CSV mesin_atm dan/atau LSBU 121" });
            continue;
          }
          const out = await updateMesinAtmMatrix({ spreadsheetId, sheetName: job.sheetName, monthLabel, rows: matrixRows });
          results.push({ job: job.name, status: "ok", sheet: job.sheetName, file: file?.name || null, ...out });
          continue;
        }

        if (job.kind === "matrix-edc") {
          if (!matrixRows?.length) {
            results.push({ job: job.name, status: "skip-matrix-no-data", reason: "Butuh CSV EDC dan/atau LSBU 0304" });
            continue;
          }
    ...
    return NextResponse.json({
      ...
      lsbu: lsbuBundles.map((b) => ({ name: b.name, kind: b.kind, rows: b.rows.length })),
