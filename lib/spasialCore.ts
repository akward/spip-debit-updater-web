...
export const SPASIAL_UE_TASKS: SpasialTask[] = [
  // Notebook Spasial_google sheet: sp_k = KOTA; Spasial UE.ipynb: lokasinasabah - pickKeyCol tries both
...
/** Prefer task.spKey, then common wilayah columns (notebook UE uses KOTA; Debet uses lokasinasabah). */
...
function resolveValue(row: Row, task: SpasialTask): number {
  ...
  if (task.mode === "col_raw" || task.mode === "vol") {
    ...
    if (e1 !== undefined && e1 !== null && e1 !== "") return num(e1);
  }
  if (task.mode === "col_juta" || task.mode === "nom") {
    const e2 = row["expr_2"] ?? row["EXPR_2"];
    if (e2 !== undefined && e2 !== null && e2 !== "") return num(e2);
  }
  return 0;
}
...
/** Final values per kota: spatial + LSBU, with /1e6 for nom modes */
...
/** Build precomputed payload for all tasks (client or server) */
...
/** Parse ArrayBuffer / rows from xlsx first sheet - used with sheet_to_json style AOA */
